...
const handler = async (m, { conn, command, args, text, usedPrefix }) => {
    if (!text) {
        throw `_𝐄𝐬𝐜𝐫𝐢𝐛𝐞 𝐮𝐧𝐚 𝐩𝐞𝐭𝐢𝐜𝐢𝐨́𝐧 𝐥𝐮𝐞𝐠𝐨 𝐝𝐞𝐥 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐞𝐣𝐞𝐦𝐩𝐥𝐨:_ \n*${usedPrefix + command} Daniel Trevor - Falling*`;
    }
...
╰ׁ̻۫─۪۬─۟─۪─۫─۪۬─۟─۪─۟─۪۬─۟─۪─۟─۪۬─۟─۪─۟┄۪۬┄۟┄۪┈۟┈۪`.trim();
        ...
        // Si es un error lanzado con throw, se envía tal cual, de lo contrario se maneja como error inesperado.
        if (e instanceof Error) {
            await conn.reply(m.chat, `*[ ! ] Ocurrió un error inesperado:* ${e.message}`, fkontak, m, rcanal);
        } else {
            await conn.reply(m.chat, e, fkontak, m, rcanal);
        }
